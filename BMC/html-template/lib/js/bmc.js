// js file used by BMC.html (generated from index.template)
var bmc = {vars:{}};
...
bmc.functions = {
	checkForOngoingProcess : function() {
		var warning_message;
		try {
			warning_message = $("#BMC")[0].hasOngoingProcess();
		}
		catch(e) {
...
bmc.utils = {
	...
	resize : function() {
		...
		doc_height = (doc_height < 590) ? 590 : doc_height; // Flash minimum
															// height is 590 px
...
bmc.mediator =  {
	...
	readUrlHash : function() {
		var module = "dashboard", // @todo: change to
									// bmc.vars.default_state.module ?
		subtab = "";
		try {
	...
	selectContent : function(uiconf_id,is_playlist) { // called by
...
bmc.preview_embed = {
	...
	doPreviewEmbed : function(id, name, description, previewOnly, is_playlist, uiconf_id, live_bitrates, has_mobile_flavors, html5_compatible) {
		...
		if(id != "multitab_playlist") {
			...
			if(bmc.vars.current_uiconf) { // set by bmc.mediator.selectContent
											// called from appstudio's "select
											// content" action
				if((is_playlist && bmc.vars.current_uiconf.is_playlist) || (!is_playlist && !bmc.vars.current_uiconf.is_playlist)) { // @todo:
																																		// minor
																																		// optimization
																																		// possible
		...
		bmc.layout.modal.open( {
			...
			'title' : id_type + ': ' + name,
			'help' : '<a class="help icon" target="_blank" href="' + bmc.vars.service_url + '/index.php/bmc/bmc2help#section118"></a>',
		...
		$("#delivery_type").change(function(){
			...
			bmc.preview_embed.doPreviewEmbed(id, name, description, previewOnly, is_playlist, uiconf_id, live_bitrates, has_mobile_flavors, html5_compatible);
		});
		$("#player_select").change(function(){
			bmc.preview_embed.doPreviewEmbed(id, name, description, previewOnly, is_playlist, this.value, live_bitrates, has_mobile_flavors, html5_compatible);
		});
	...
	buildRtmpOptions : function() {
		...
		'while Progressive Download allows buffering of the content. <a target="_blank" href="' + bmc.vars.service_url + '/index.php/bmc/bmc2help#1431">Read more</a></div><br />';
		return html;
	},
	...
	buildHTML5Option : function(entry_id, partner_id, uiconf_id, has_mobile_flavors) {
		...
		var description = '<div class="note red">This video does not have video flavors compatible with IPhone & IPad. <a target="_blank" href="' + bmc.vars.service_url + '/index.php/bmc/bmc2help#section1432">Read more</a></div>';
		if(has_mobile_flavors) {
			description = '<div class="note">If you enable the HTML5 player, the viewer device will be automatically detected.' +
			' <a target="_blank" href="' + bmc.vars.service_url + '/index.php/bmc/bmc2help#section1432">Read more</a>' +
	...
	doFlavorPreview : function(entry_id, entry_name, flavor_details) {
		...
		bmc.layout.modal.open( {
			...
			'height' : parseInt(bmc.vars.default_bdp.height) + 200,
	...
	// eventually replace with <? php echo $embedCodeTemplate; ?> ; (variables
	// used: HEIGHT WIDTH HOST CACHE_ST UICONF_ID PARTNER_ID PLAYLIST_ID
	// ENTRY_ID) + {VER}, {SILVERLIGHT}, {INIT_PARAMS} for Silverlight + NAME,
	// DESCRIPTION
	embed_code_template :	{
	...
	// id = entry id, asset id or playlist id; name = entry name or playlist
	// name;
	// uiconf = uiconfid (normal scenario) or uiconf details json (for
	// #content|Manage->drill down->flavors->preview)
	buildBorhanEmbed : function(id, name, description, is_playlist, uiconf, html5) {

		var uiconf_id = uiconf.uiconf_id || uiconf,
		uiconf_details = (typeof uiconf == "object") ? uiconf : bmc.preview_embed.getUiconfDetails(uiconf_id,is_playlist),  // getUiconfDetails
																															// returns
																															// json
		...
		if(is_playlist && id != "multitab_playlist") {	// playlist (not
														// multitab)
			...
			// bmc.log(uiconf_details.swf_version);
			// alert("uiconf_details.swf_version logged");
		...
		embed_code = embed_code.replace("{ALT}", ((bmc.vars.whitelabel) ? "" : bmc.preview_embed.embed_code_template.borhan_links));
	...
	getUiconfDetails : function(uiconf_id,is_playlist) {
		...
		$("#BMC")[0].alert("getUiconfDetails error: uiconf_id "+uiconf_id+" not found in " + ((is_playlist) ? "bmc.vars.playlists_list" : "bmc.vars.players_list"));
		return false;
	},
	setCacheStartTime : function() {
		var d = new Date;
		cache_st = Math.floor(d.getTime() / 1000) + (15 * 60); // start caching
																// in 15 minutes
		return cache_st;
	},
...
bmc.client = {
	...
	buildClientURL : function(service, action) {
		// return bmc.vars.service_url +
		// '/api_v3/index.php?service='+service+'&action='+action;
	...
	createShortURL : function(url) {
		...
		var data = {
			...
			"shortLink:systemName"	: "BMC-PREVIEW", // Unique name for
														// filtering
...
function openPlayer(emptystring, width, height, uiconf_id, previewOnly) { 
	if (previewOnly==true) $("#BMC")[0].alert('previewOnly from studio');
	bmc.preview_embed.doPreviewEmbed("multitab_playlist", null, null, previewOnly, true, uiconf_id); 
}
...
// If we have ongoing process, we show a warning message when the user try to
// leaves the page
...
bmc.user = {
	...
	logout: function() {
		var expiry = new Date("January 1, 1970"); // "Thu, 01-Jan-70 00:00:01
													// GMT";
...
/*
 * XD: a backwards compatable implementation of postMessage
