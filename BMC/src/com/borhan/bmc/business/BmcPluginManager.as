package com.borhan.bmc.business
{
	...
	public class BmcPluginManager extends EventDispatcher {
		
		/**
		 * BMC uiconf, holds data regarding modules and plugins 
		 */
		private var _uiconf:XML;
		
		/**
		 * associative array of plugins (FlexModules) that BMC loaded, 
		 * listed by their ids
		 * */
		...
		public function BmcPluginManager(approot:DisplayObjectContainer, client:BorhanClient, flashvars:Object, eventHandlers:Object)
		{
			_approot = approot;
			_client = client;
		...
		private function onPluginLoaded (e:ModuleEvent):void {
			...
			for (var i:uint = 0 ; i< atts.length() ; i++) {
				...
				if (att != "path" && att != "dependencies") {
					// the above are required by BMC, not by the plugin
			...
			if (plugin is IBmcPlugin) {
				(plugin as IBmcPlugin).client = _client;
				(plugin as IBmcPlugin).flashvars = _flashvars;
			}
		...
		/**
		 * dispatch a BMCErrorEvent
		 * @param e
		 */
		private function onPluginLoadError (e:ModuleEvent):void {
			dispatchEvent(new BmcErrorEvent(BmcErrorEvent.ERROR, e.errorText));
		}
		
		/**
		 * load required BMC plugins 
		 * */
		...
		public function executePluginMethod(pluginId:String, methodName:String, ...args):* {
			try {
				...
				dispatchEvent(new BmcErrorEvent(BmcErrorEvent.ERROR, ResourceManager.getInstance().getString('bmc', 'method_dont_exist', [pluginId, methodName])));
			}
		}
