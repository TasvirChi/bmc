package com.borhan.bmc.modules.analytics.commands
{
	...
	import com.borhan.commands.report.ReportGetBaseTotal;
	...
	public class GetBaseTotalsCommand implements ICommand, IResponder
	{
		...
		public function execute(event:CairngormEvent):void
		{
			...
			if (!_model.selectedReportData.pager) {
				_model.selectedReportData.pager = new BorhanFilterPager();
			...
			var filter:BorhanEndUserReportInputFilter = ExecuteReportHelper.createEndUserFilterFromCurrentReport(_model.getFilterForScreen((event as ReportEvent).screenType));
			...
			getBaseTotals.addEventListener(BorhanEvent.COMPLETE, result);
			getBaseTotals.addEventListener(BorhanEvent.FAILED, fault);
