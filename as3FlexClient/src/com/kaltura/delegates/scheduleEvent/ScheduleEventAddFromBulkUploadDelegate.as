...
// @ignore
// ===================================================================================================
package com.borhan.delegates.scheduleEvent
{
	...
	import com.borhan.commands.scheduleEvent.ScheduleEventAddFromBulkUpload;
	...
	public class ScheduleEventAddFromBulkUploadDelegate extends WebDelegateBase
	{
		protected var mrloader:MultipartURLLoader;

		public function ScheduleEventAddFromBulkUploadDelegate(call:BorhanCall, config:BorhanConfig)
		{
			super(call, config);
		}

		override public function parse(result:XML):* {
			if ((call as ScheduleEventAddFromBulkUpload).fileData is FileReference) {
		...
		override protected function sendRequest():void {
			...
			if ((call as ScheduleEventAddFromBulkUpload).fileData is FileReference) {
				(call as ScheduleEventAddFromBulkUpload).fileData.addEventListener(DataEvent.UPLOAD_COMPLETE_DATA,onDataComplete);
				var urlRequest:URLRequest = new URLRequest(req);
				((call as ScheduleEventAddFromBulkUpload).fileData as FileReference).upload(urlRequest,"fileData");
			}
			else {
				mrloader.addFile(((call as ScheduleEventAddFromBulkUpload).fileData as ByteArray), UIDUtil.createUID(), 'fileData'); 
