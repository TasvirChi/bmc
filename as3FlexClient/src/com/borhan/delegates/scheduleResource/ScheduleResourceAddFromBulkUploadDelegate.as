...
// @ignore
// ===================================================================================================
package com.borhan.delegates.scheduleResource
{
	...
	import com.borhan.commands.scheduleResource.ScheduleResourceAddFromBulkUpload;
	...
	public class ScheduleResourceAddFromBulkUploadDelegate extends WebDelegateBase
	{
		protected var mrloader:MultipartURLLoader;

		public function ScheduleResourceAddFromBulkUploadDelegate(call:BorhanCall, config:BorhanConfig)
		{
			super(call, config);
		}

		override public function parse(result:XML):* {
			if ((call as ScheduleResourceAddFromBulkUpload).fileData is FileReference) {
		...
		override protected function sendRequest():void {
			...
			if ((call as ScheduleResourceAddFromBulkUpload).fileData is FileReference) {
				(call as ScheduleResourceAddFromBulkUpload).fileData.addEventListener(DataEvent.UPLOAD_COMPLETE_DATA,onDataComplete);
				var urlRequest:URLRequest = new URLRequest(req);
				((call as ScheduleResourceAddFromBulkUpload).fileData as FileReference).upload(urlRequest,"fileData");
			}
			else {
				mrloader.addFile(((call as ScheduleResourceAddFromBulkUpload).fileData as ByteArray), UIDUtil.createUID(), 'fileData'); 
