package com.borhan.edw.components.fltr.cat.renderers {
	 ...
	 *  CatTreeSelectionMode.SINGLE_SELECT mode
	 * @author Atar
	 * 
	 */
	public class CatTreeSingleIR extends TreeItemRenderer implements ICatTreeRenderer {
		...
		private function onToggle(e:Event):void {
			e.stopPropagation();
//			var listOwner:CatTree = CatTree((listData as TreeListData).owner);
//			data[listOwner.selectionAttribute] = cb.selected ? CatSelectionStatus.SELECTED : CatSelectionStatus.UNSELECTED;
		...
		/**
		 * set RB selection status according to new cat selection status
